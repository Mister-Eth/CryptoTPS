...
namespace CryptoTPS.Services.BlockchainServices.Status.BackgroundTasks.Discord
{
    public class PlausibleVisitorCountBackgroundTask : BackgroundTaskWithNotifier
    {
        ...
        public PlausibleVisitorCountBackgroundTask(ILogger<HangfireBackgroundService> logger, CryptoTPSContext context, IConfiguration configuration) : base(logger, context, configuration)
        {
        ...
        public override async Task RunAsync()
        {
            ...
            var response = await _httpClient.GetAsync(new Uri($"{_url}/api/v1/stats/aggregate?site_id=CryptoTPS.info&period=day&metrics=visitors,pageviews&date={DateTime.Now.Year}-{DateTime.Now.Month}-{DateTime.Now.Subtract(TimeSpan.FromDays(1)).Day}", UriKind.Absolute));
